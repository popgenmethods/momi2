...
def test_admixture_demo():
    ...
    empirical_sfs,sqCounts = run_scrm([sum([v for k,v in leaf_lins.iteritems()]), 
                                       num_scrm_samples, 
                                       '-t', theta, 
    ...
    print(p_val)
    assert p_val > .01, {key: (empirical_sfs[key], theoretical_sfs[key] * theta / 2.0 * num_scrm_samples) for key in empirical_sfs}
