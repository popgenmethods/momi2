...
def simple_admixture_demo(x=np.random.normal(size=7), n_lins=(2,3)):
    ...
    #return make_demography(events, sampled_pops = ('b','a'), sampled_n = n_lins, default_N = 1e4)
...
def simple_admixture_3pop(x=None, n_lins=(4,4,4)):
    ...
    #return make_demography(events, sampled_pops = ('b','a','c'), sampled_n = n_lins)    
...
def simple_two_pop_demo(x=np.random.normal(size=4), n_lins=(5,6)):
    ...
    events = [('-en',0.,1,x[1]), ('-en',0.,0,x[2]), ('-ej',x[0],0,1), ('-en',x[0],1,x[3])]
    #return make_demography(events, sampled_pops = (1,0), sampled_n = n_lins, default_N = 1e4)
...
def piecewise_constant_demo(x = np.random.normal(size=15), n_lins = (10,)):
    ...
    #return make_demography(events_list, sampled_pops = (0,), sampled_n = n_lins, default_N = 1e4)


def exp_growth_model(x = np.random.normal(size=3), n_lins = (10,)):
    t,g,g2 = x
    t,g2 = np.exp(t), np.exp(g2)
    events = [('-eg',0.,0,g/1e4),('-eg',t*1e4,0,g2/1e4),('-eg',3*t*1e4,0,0.)]
    return make_demo_hist(events, sampled_pops = (0,), sampled_n = n_lins, default_N = 1e4)
    #return make_demography(events, sampled_pops = (0,), sampled_n = n_lins, default_N = 1e4)
...
def simple_five_pop_demo(x = np.random.normal(size=30), n_lins=(1,2,3,4,5)):
    ...
    demo = make_demo_hist(events_list, sampled_pops = list(range(1,len(n_lins)+1)), sampled_n = n_lins)
