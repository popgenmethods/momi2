...
dt.summary <- dt[, list(avg_t=mean(time),runs=length(time),negs=sum(result < 0,na.rm=TRUE),nans=sum(is.na(result))), by=list(model, n, D, initial)]

p <- ggplot(dt.summary, aes(x=n, y=avg_t, color=D)) + geom_line() + geom_point() + ylab("Seconds") + theme(legend.position="top") + facet_grid(initial ~ model, scales='free_y')
plot(p)
ggsave('figures/timing.pdf')

p <- ggplot(dt.summary, aes(x=n, y=avg_t, color=D, linetype=model)) + geom_line() + geom_point(aes(shape=model)) + ylab("Seconds") + theme(legend.position="top") + facet_grid(initial ~ .) + scale_y_log10(breaks=c(1e-4,1e-2,1,1e2,1e4)) + scale_x_log10(breaks=c(2,4,8,16,32,64,128,256))
plot(p)
ggsave('figures/timing_log.pdf')

#dt$transformed.result <- sign(dt$result) * log10(1 + abs(dt$result))

dt2 <- dcast(dt, n + D + site + run_id + state + tree + initial ~ model, value.var = "result")
