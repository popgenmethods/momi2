...
def seg_sites_from_ms(ms_file, sampled_pops):
    ...
    if " -T " in lines[0]:
        ...
        raise NotImplementedError(
            "Reading SFS from ms file with -T not yet implemented")

    # don't split up escaped spaces in /ms/path
    firstline = lines[0].replace('\ ', '\_').split()
...
def simulate_ms(ms_path, demo, num_loci, mut_rate, seeds=None, additional_ms_params="", cmd_format="ms", raw_output=False, sampled_pops=None, sampled_n=None):
    ...
    if any([(x in additional_ms_params) for x in ("-t", "-T", "seed")]):
        raise IOError(
            "additional_ms_params should not contain -t,-T,-seed,-seeds")
...
def to_ms_cmd(demo, cmd_format="ms"):
    ...
    if demo.default_N != 1.0:
        raise Exception(
            "Please rescale demography to be in ms units (i.e. default_N=1.0). This can be done with Demography.rescaled().")
    ...
    if cmd_format == "ms" and not all(sampled_t == 0.0):
        raise Exception(
            "ms command line doesn't allow for archaic leaf populations. Try setting cmd_format='scrm'")

    pops = dict(list(zip(demo.sampled_pops, list(
        range(1, len(demo.sampled_pops) + 1)))))
...
def _convert_ms_cmd(cmd_list, params):
    ...
    if cmd_list[0][0] != 'I':
        raise IOError(
            "ms command must begin with -I to specify samples per population")
    ...
    pops_map = dict(
        list(zip(pops_by_time, list(range(1, len(pops_by_time) + 1)))))
    for cmd in cmd_list:
    ...
    for cmd in cmd_list:
        if cmd[0] == "I":
            if len(cmd[2:]) != int(cmd[1]):
                raise IOError(
                    "Wrong number of arguments for -I (note continuous migration is not allowed)")
            del cmd[1]
            cmd[0] = "n"
